...
function f4() {
	let out = '';
	for (let key in a4) out += `${key} ${a4[key]} <br>`;
	return out;
}
...
function f13() {
	let q = 0;
	for (let key in a13) {
		if (typeof a13[key] === 'number') q = q + a13[key];
	}
	document.querySelector('.out-13').innerHTML = q;
}
...
function f14() {
	...
	document.querySelector('.out-14').innerHTML = q;
}
...
function f15() {
	let q = '';
	for (let key in a15) {
		for (let t = 0; t < a15[key].length; t++) {
			q += `${a15[key][t]} `;
		}
	}
	document.querySelector('.out-15').innerHTML = q;
}
...
function f16() {
	...
	document.querySelector('.out-16').innerHTML = q;
}
...
function f17() {
	let q = '';
	for (let key in a17) {
		if (a17[key].age > 30) q += `${a17[key].name} `;
	}
...
function f18() {
	...
	if (a18[i18] !== undefined) {
		console.log(a18[i18]);
		for (let i = 0; i < a18[i18].length; i++) q += `${a18[i18][i]} `;
	}

	document.querySelector('.out-18').innerHTML = q;
}
...
function f19() {
	...
	for (let key in a19) {
		...
		document.querySelector('.out-19').innerHTML = q;
	}
}
...
function f20() {
	...
	for (let key in a20) {
		for(i = 0; i < a20[key].length; i++) {
			if (a20[key][i][1] === 2) q += `${a20[key][i][0]} `;
		}
	}
	document.querySelector('.out-20').innerHTML = q;
}
